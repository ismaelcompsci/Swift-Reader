...
public class NoContextMenuPDFView: PDFView {
    ...
    @objc func tappedGestureHandler(sender: UITapGestureRecognizer) {
        if sender.state == .ended {
            ...
            let point = convert(tapPoint, to: currentPage)

            if let tappedAnnotation = currentPage.annotation(at: point) {
                switch tappedAnnotation.type {
                case "Link":
                    if let action = tappedAnnotation.action {
                        viewModel?.pdfView.perform(action)
                    }
                case "Highlight", "Underline":
                    guard let id = tappedAnnotation.annotationKeyValues[PDFAnnotationKey.highlightId] as? String else { return }
                    viewModel?.highlightHit(id)
                default:
                    print("UNSUPPORTED", tappedAnnotation.type ?? "")
                }
            } else {
                viewModel?.onTap.send(point)
            }
