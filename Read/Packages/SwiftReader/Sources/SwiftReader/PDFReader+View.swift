...
public class NoContextMenuPDFView: PDFView {
    ...
    @objc func tappedGestureHandler(sender: UITapGestureRecognizer) {
        if sender.state == .ended {
            ...
            if let tappedHighlight = tappedHighlightsValues?.first,
               ...
               let tappedUUIDString = tappedAnnotation.value(forAnnotationKey: PDFAnnotationKey(rawValue: "highlightId")) as? String,
    ...
    override public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
